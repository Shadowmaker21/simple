...
$sanksi = stripslashes($hasil['sanksi']);
...
if (isset($_POST['Edit'])) {
	...
	$query = "UPDATE surat SET jenis_srt='$jenis_srt', sanksi='$sanksi', nomor_srt='$nomor_srt',
			  ket='$ket', WHERE id='$id'";
	$sql = mysqli_query($conn, $query);
	if ($sql) {
	...
	<FORM ACTION="" METHOD="POST" NAME="input" enctype="multipart/form-data">
		<table cellpadding="0" cellspacing="0" border="0" width="700">
			...
			<tr>
				...
				<td>:
					<select name="sanksi">
						<option>-- Pilih Jenis Sanksi --</option>
						<option value="Teguran" <?= ($jenis_srt == 'Teguran') ? 'selected' : '' ?>>Teguran</option>
						<option value="Tertulis" <?= ($jenis_srt == 'Tertulis') ? 'selected' : '' ?>>Tertulis</option>
					</select>
				</td>
			</tr>
			<br>
